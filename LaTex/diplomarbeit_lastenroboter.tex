...
\begin{document}
	...
	Zusätzlich wird eine OnBoard-Software entwickelt, die es ermöglicht, die Sensoren auszulesen und sowohl die Motoren als auch die Kamera anzusteuern.\\[0.5 cm]
	...
	\section{Projektmanagement}
		...
		Unsere Diplomarbeit beschäftigt sich mit der Entwicklung eines Lastenroboters. Der Lastenroboter soll etwa ein Gewicht von 25 Kilogramm tragen können. Die Steuerung erfolgt über eine Website, und zusätzlich soll eine Kamera eingebaut werden. Im Lastenroboter wird auch eine Waage eingebaut, damit man sehen kann, wie schwer die transportierte Last ist. \\ Das Ziel dieser Diplomarbeit ist also, einen betriebsbereiten Prototyp eines Lastenroboters mit Kamerasystem und Waage zu entwickeln und zu realisieren. \\[0.4cm]
		...
		\begin{center}
			\begin{minipage}{\textwidth}
				...
				\captionof{figure}{Lastenroboter Projekt-Plan}
	...
	\section{Antrieb}
		...
		\subsection{Motoren} %Felix
			...
			\textbf{Rotor:} Der Rotor besteht aus einem Permanentmagneten. Dadurch, dass die Spulen ein rotierendes Magnetfeld erzeugen, dreht sich der Rotor synchron mit diesem Magnetfeld mit.
		...
		\begin{itemize}
			\item Die Platine verfügt über zwei Steuereingänge, einen für Richtungswechsel (DIR) und eine Bremsfunktion (BRAKE).
		...
		\subsection{Schaltungsaufbau mit vier Motoren} %Felix
			...
			\newpage \noindent
		\subsection{Code} 
		\fancyfoot[L]{Felix Hochegger \& Daniel Schauer}
	...
	In der Funktion onWebSocketEvent() werden die WebSocket-Ereignisse verarbeitet. Sie wird aufgerufen, wenn sich ein WebSocket-Client verbindent, eine Nachricht sendet oder die Verbindung getrennt wird.
	...
	Um den Roboter überhaupt steuern zu können, wird ein Steuerkreuz implementiert. Das Steuerkreuz befindet sich mittig am rechten Bildschirmrand. Das Steuerkreuz besteht aus fünf Tasten, nämlich: Vorwärts (\texttt{UP}), (\texttt{DOWN} ), links (\texttt{LEFT}), rechts (\texttt{RIGHT}) und in der Mitte Stop (\texttt{HALT}). \\[0.5cm]
	\begin{center}
	...
	Die \texttt{active-light} Klasse ändert die Hintergrundfarbe zu einem transparenten grünen Ton, um anzuzeigen, dass das Fernlicht aktiv ist. 
	\begin{center}
		\begin{minipage}[t]{0.65\textwidth}
	...
	\begin{center}
		...
		\end{minipage} \\[0.75cm]
	\end{center}
	In der CSS-Klasse \texttt{mymenu} wird die sidebar formatiert. Das Menü ist fixiert und nimmt 100\% der Höhe ein. Die Anfängliche Höhe ist 0, da es versteckt bzw. eingeklappt ist. Die Hintergrundfarbe ist schwarz und das Menü liegt auf der ersten Ebene über dem Hauptinhalt, damit es diesen überlagern kann, wenn es eingeblendet wird. Beim Ein- und Ausblenden dauert 0.5 Sekunden, um einen sanften Übergang zu ermöglichen. \\[0.5cm]
...
Im HTML-Code wird die Status-Box als HTML \texttt{<span>}-Tag definiert. Für das Design wird die CSS-Klasse \texttt{status-box} kombiniert je nach Verbindungsstatus mit der Klasse \texttt{connected} oder \texttt{disconnected}. 
	...
	\fancyfoot[L]{Felix Hochegger}
		...
		\begin{enumerate}
			...
			\item Größe: Bei der Größe ist es wichtig darauf zu achten das alle Komponenten problemlos ins Gehäuse passen.
		...
		\begin{center} 
			\begin{minipage}[t]{0.6\textwidth}
				...
				\captionof{figure}{Fertiges Modell}
		...
		\begin{center} 
			\begin{minipage}[t]{0.53\textwidth}
				\includegraphics[scale=0.2]{Pictures/oberen Rahmen}
				\captionof{figure}{Oberer Rahmen}
				\label{fig:modell-fertig}
				\vspace{-10pt}
				\begin{center}
					\par\small Quelle: eigene Abbildung 
	...
	\begin{center} 
		\begin{minipage}[t]{0.33\textwidth}
			...
			\captionof{figure}{Motorbefestigung}
			\label{fig:modell-fertig}
			\vspace{-10pt}
			\begin{center}
				\par\small Quelle: eigene Abbildung 
	...
	\end{center} 
	 ...
	 \begin{center} 
	 	\begin{minipage}[t]{0.3\textwidth}
	 		\includegraphics[scale=0.07]{Pictures/Montierung-wägezelle}
	 		\captionof{figure}{Integration der Wägezelle}
	 		\label{fig:modell-fertig}
	 		\vspace{-10pt}
	 		\begin{center}
	 			\par\small Quelle: eigene Abbildung 
	 ...
	 \begin{center} 
	 	\begin{minipage}[t]{0.5\textwidth}
	 		...
	 		\captionof{figure}{Erster funktionierender Prototyp}
	...
	Zusätzlich zur Steuerplatine umfasst das System auch externe Komponenten wie die Batterie und den Dino KRAFTPAKET 4A-6V/12V Batterieladegerät. Diese wurden separat verbaut, sind jedoch nahtlos in das Gesamtsystem integriert, um eine stabile Energieversorgung, effizientes Laden und die notwendige Spannungsumwandlung zu gewährleisten. Diese externen Bauteile sind ebenfalls ein wichtiger Bestandteil des Hardwaredesigns, auch wenn sie nicht direkt auf der Steuerplatine untergebracht sind.
		...
		\end{equation}
		Für die Platine wurde ein Spannungsregler entwickelt der 12 Volt auf 5 Volt herunterregelt. Das heißt Vout soll 5 Volt betragen. Damit man die Widerstände bestimmen kann, muss bei einem Widerstand ein Wert angenommen werden. In diesem Fall wurde für R1 der Wert von 270 Ohm festgelegt.
		\newpage
		\begin{figure}[htbp]
			\begin{minipage}[t]{6cm}
				\vspace{0pt}
		...
		\subsubsection*{4) VCC\_5V (5V Spannungsversorgung)}
		...
		Der ESP32 misst die Batteriespannung und liest die Daten des HX711 ein. Er verfügt auch über eine I2C-Schnittstelle, die für die Wägezelle und das Expander-Board verwendet wird. Zusätzlich steuert der ESP32 die PWM-Signale für die Motorentreiber, um die Drehzahl der Motoren präzise zu regulieren.
		...
		\subsubsection{8. Expander\_Anschlüsse (GPIO-Erweiterung über MCP23017)}
		...
		\subsubsection*{Berechnung des Basiswiderstands:}
	...
	\begin{center} 
		\begin{minipage}[t]{0.85\textwidth}
			...
			\captionof{figure}{Steuerungsplatine}
			\label{fig:i2c-anschlüsse}
			\vspace{-10pt}
			\begin{center}
				\par\small Quelle: eigene Abbildung
	...
	\section{Kamera}
		...
		\subsection{Code} %Daniel und %Simon
			...
			\newpage \noindent 
			\fancyfoot[L]{Daniel Schauer \& Simon Spari}
			\begin{center}
				\begin{minipage}[t]{1\textwidth}
			...
			\begin{center}
				\begin{minipage}[t]{0.75\textwidth}
					...
					\captionof{figure}{Halterung der Kamera}
		...
		\subsection{Schwenkmechanismus der Kamera}
				...
				Der SG90 ist ein per PWM-Signal gesteuerter Servomotor, dessen Position durch einen Winkel zwischen 0° und 180° festgelegt wird. Zusätzlich gibt es Servomotoren mit einem Drehbereich von 360°, die in der Richtung als auch  in der Drehgeschwindigkeit gesteuert werden können.
				...
				\begin{center}
					\begin{minipage}[t]{0.7\textwidth}
						\includegraphics[scale=0.3]{Pictures/Kameragehäuse}
						\captionof{figure}{Kameragehäuse}
						\label{fig:camturn}
						\vspace{-10pt}
						\begin{center}
							\par\small Quelle: eigene Abbildung 
				...
				\begin{center}
					\begin{minipage}[t]{0.2\textwidth}
						...
						\captionof{figure}{Aussparung für den Servomotor}
						\label{fig:camturn}
						\vspace{-10pt}
						\begin{center}
							\par\small Quelle: eigene Abbildung
	 ...
	 \end{center}
		...
		\begin{center}
			\begin{minipage}{0.8\textwidth}
				...
				\captionof{figure}{Pin und Variable für Lichtsteuerung}
		...
		\begin{center}
			\begin{minipage}{0.8\textwidth}
				...
				\captionof{figure}{\texttt{Setup();} - Code für Lichtsteuerung}
		...
		\begin{center}
			\begin{minipage}{0.8\textwidth}
				...
				\captionof{figure}{\texttt{onWebSocketEvent();} - Code für Lichtsteuerung}
		...
		\begin{center}
			\begin{minipage}{0.8\textwidth}
				...
				\captionof{figure}{Funktionen zum Ein-/Auschalten des Lichtes}
	...
	\fancyfoot[L]{Felix Hochegger \& Simon Spari}
	 ...
	 \newpage \noindent
	 	\subsubsection{Setup}
	 \fancyfoot[L]{Simon Spari \& Daniel Schauer}
	...
	Die HX711\_ADC-Bibliothek ermöglicht die Nutzung des HX711 ADC-Moduls mit Arduino und ESP32 zur präzisen Messung von Gewichtssensoren (Wägezellen). Sie bietet eine verbesserte Signalverarbeitung, Tare-Funktion und Kalibrierungsfunktionen. Die Bibliothek erleichtert das Auslesen der Sensordaten und sorgt für eine stabile Gewichtsmessung, ohne komplexe Signalverarbeitung manuell durchführen zu müssen. \footnote{https://github.com/olkal/HX711\_ADC}
	 ...
	 \subsection{LaTex}
	 LaTeX ist ein Textsatzsystem, das besonders für die Erstellung von wissenschaftlichen Arbeiten, technischen Dokumentationen und anderen anspruchsvollen Texten verwendet wird. Es basiert auf dem Prinzip der Auszeichnungssprache, bei dem Inhalt und Formatierung getrennt voneinander behandelt werden. Dadurch lassen sich Dokumente sehr präzise und konsistent gestalten.\\[0.5cm]
	 Wir haben für unser Projekt LaTeX benutzt, um die Dokumentation zu schreiben. Dabei haben wir das Programm TeXstudio \footnote{https://www.texstudio.org/} verwendet. Es hilft uns, den Text zu schreiben und gleich zu sehen, wie das fertige Dokument aussieht.\\[0.5cm]
